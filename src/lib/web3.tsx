...
export function WalletProvider({ children }: { children: React.ReactNode }) {
    ...
    const handleWalletClick = async (wallet: string) => {
        ...
        // ============================================================
        // TELEGRAM MINI APP — Use WalletConnect with universal links
        // WC sessions persist across app switches (unlike dapp browser).
        // URI is generated by prepareWalletConnect() → displayed via activeUri useEffect.
        // ============================================================
        if (isTMA) {
            // Quick check: if injected provider is already available (user may be in wallet's dapp browser)
            const eth = (window as any).ethereum;
            if (eth?.request) {
                try {
            ...
            // TMA: Use WalletConnect flow (NOT dapp browser approach)
            // WC sessions persist when user switches apps and comes back to TMA.
            // The dapp-browser approach loses connection on app switch.
            // prepareWalletConnect() was already called when modal opened → URI will be generated
            // The activeUri useEffect will handle opening the WC deeplink automatically.
            setConnectingWallet(wallet);
            setWalletType(wallet);
            localStorage.setItem('aimining_wallet_type', wallet);
            console.log('[TMA] Using WalletConnect flow for:', wallet);
            return;
        }
